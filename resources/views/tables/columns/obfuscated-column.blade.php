...
<div x-data="{ revealed: false }" @click="revealed = !revealed" class="flex items-center gap-2 cursor-pointer group">
    <span class="text-sm" x-show="!revealed">{{ $obfuscated }}</span>
    <span class="text-sm" x-show="revealed" x-cloak>{{ $state }}</span>
    @if ($state !== null && $state !== '')
        <svg class="w-4 h-4 text-gray-400 group-hover:text-primary-500" x-bind:class="{ 'text-primary-500': revealed }"
